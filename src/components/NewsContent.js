...
const NewsContent = ({ setLoadMore, loadMore, newsArray, newsCount }) => {
  ...
  // Filter newsArray to only include items with an image and description
  const filteredNewsArray = newsArray.filter(newsItem => newsItem.urlToImage && newsItem.description)
